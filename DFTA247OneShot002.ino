...
20 bleu  = surtension
20 R/B clignotant = inversion ou Vbat < 50 mV
20 rouge fixe = fin alarme surtension
...
#define RSENSE   2.2f      // Résistance de mesure de courant
...
void setup() {
  ...
  if (Vinv < 0.05f) { // ≈ tension zéro → inversion ou aucune pile
    ...
    // Clignotement rouge des LED 4 / 12 / 20
  ...
  strip.setPixelColor(4,          // LED stabilité
  ...
  strip.setPixelColor(8 + nc, colC);

  // LED de stabilité barrette 2
  strip.setPixelColor(12,
  ...
  for (int i = 16; i <= 23; i++) strip.setPixelColor(i, 0);
  ...
  if (Vvide > 1.65f) {
    ...
    for (int i = 0; i < 12; i++) {
      strip.setPixelColor(20, strip.Color(0,0,200)); // bleu
      strip.show();
      delay(100);

      strip.setPixelColor(20, strip.Color(200,0,0)); // rouge
      strip.show();
      delay(100);
    }
  ...
  // ----------- ANALYSE NORMALE ----------
    ...
    if (nv==2 && nc==2 && deltaV < 0.25f)
      strip.setPixelColor(18, strip.Color(0,200,0));

    // Cas moyen : cohérent mais ΔV modéré
    else if (nv>0 && nc>0 && deltaV < 0.30f)
      strip.setPixelColor(17, strip.Color(255,120,0));
